...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ *Aguarde um pouco, caso não dê certo, tente novamente!* ⌛',
				success: 'Com Sucesso ✔️',
				error: {
					stick: '❌ Ocorreu um erro ao converter a imagem/gif em figurinha ❌',
					Iv: '❌ Link tidak valid ❌'
				},
				only: {
			...
			switch(command) {
				...
				case 'sticker':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie foto ou gif com legenda ${prefix}sticker ou tags de imagem/gif que já foram enviadas`)
					}
					break
					case 'gtts':
				...
				case 'gangster':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Frio e calculista 🤫'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aproveite enquanto Chamo a Polica Federal!'})
					})
					break
				case 'nsfwloli':
        ...
        case 'promote':
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('A tag alvo que você deseja compartilhar! ')')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'SE FODEU! :\n Foi removido com sucesso ✔️'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
